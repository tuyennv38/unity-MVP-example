...
│   └── Tests/                       ← Unit test đóng gói cùng Coordinator
│       └── GameCoordinatorTests.cs
├── Features/
│   ├── FeatureA/                    ← Module MVP #1 (có Tests/ riêng)
...
## Unit Test cho Coordinator — Đóng gói cùng module

> **BẮT BUỘC.** Test đặt trong `Core/Tests/` để đi cùng với Coordinator.

### Cách test Coordinator

Coordinator test kiểm tra: **khi event X xảy ra, Coordinator gọi đúng method trên đúng module**.

Dùng **Mock Presenter** (giống Mock View trong MVP) cho từng module:

```csharp
// Mock từng Presenter — ghi lại mọi lời gọi
public class MockInventoryPresenter : IInventoryPresenter
{
    public string? LastAddedItem;
    public bool DropAllCalled;

    public void AddItem(string itemId) => LastAddedItem = itemId;
    public void DropAllItems() => DropAllCalled = true;
}
...
public class MockHUDPresenter : IHUDPresenter
{
    ...
    public void UpdateGold(int gold) => LastGold = gold;
    public void ShowWarning(string msg) => LastWarning = msg;
...
public class GameCoordinatorTests
{
    ...
    public void HandleItemPurchased_UpdatesAllModules()
    {
        ...
        Assert.That(mockInventory.LastAddedItem, Is.EqualTo("sword_01"));
        Assert.That(mockHUD.LastGold, Is.EqualTo(400));
    }

    [Test]
    public void HandleInventoryFull_DisablesBuyAndShowsWarning()
    {
        var mockShop = new MockShopPresenter();
        var mockHUD = new MockHUDPresenter();

        // Simulate
        mockShop.SetBuyEnabled(false);
        mockHUD.ShowWarning("Inventory is full!");

        Assert.That(mockShop.BuyEnabled, Is.False);
        Assert.That(mockHUD.LastWarning, Is.EqualTo("Inventory is full!"));
    }
}
```

**Checklist Test Coordinator:**
- ✅ Đặt trong `Core/Tests/` — đóng gói cùng Coordinator
- ✅ Tên file: `<CoordinatorName>Tests.cs`
- ✅ Tạo **Mock Presenter** cho mỗi module liên quan
- ✅ Mỗi test kiểm tra **1 luồng event** (VD: ItemPurchased → 3 hành động)
- ✅ Test runner: chạy qua `dotnet test Tests/Tests.csproj` từ root project
- ✅ Thêm link vào `Tests.csproj` khi thêm Coordinator mới
